...
function Fix-File($path) {
    if (Test-Path $path) {
        ...
        # 1. Force local logo.png reference
        ...
        if ($path -like "*detailed.html") {
            ...
            # Additional overlap fixes for the Oracle/Reserve area
            $content = $content.Replace('Oracle -- "Locks Collateral" --> Reserve', 'Oracle -- "<br/>Locks Collateral<br/>" --> Reserve')
            $content = $content.Replace('Oracle -. "Deplete Collateral" .-> Reserve', 'Oracle -. "<br/>Deplete Collateral<br/>" .-> Reserve')
        }
