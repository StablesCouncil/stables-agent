...
**Demo channel** (`prod_stables_app_demo`): forked from showcase **`prod_stables_app_v00.00.00.00.03`** (legacy folder name `prod_stables_app_v00.00.02`). Full label **`v00.00.00.01.00`**, **`APP_STAGE: demo`**, top bar **Demo · v…**. Log **demo-only** and **shared** UI work here; **showcase-only** fixes stay in **`prod_stables_app_v00.00.00.00.03/CHANGELOG.md`**. See **`0_handshake/minidapp_version.md`** for routing.
