...
- [Article IX — AI Agents and Integration](stables_charter.md#article-ix--ai-agents-and-integration)  
  ...
  - [IX.2 Open Integration and Invariant Boundaries](stables_charter.md#ix2-open-integration-and-invariant-boundaries)  
  - [IX.3 Disclosure, Provenance, and Conduct](stables_charter.md#ix3-disclosure-provenance-and-conduct)  
  - [IX.4 Council Agent and Third-Party Agents](stables_charter.md#ix4-council-agent-and-third-party-agents)  
- [Article X — Final Declaration](stables_charter.md#article-x--final-declaration)  
...
## ARTICLE IX — AI Agents and Integration

**Section summary:** This Article recognises that **artificial intelligence systems and autonomous agents** will increasingly **integrate with, explain, and operate alongside** Stables. It states that such integration is **welcome and expected**, sets **invariant boundaries** (agents do not override the monetary core), and requires **honest disclosure** of agent identity, limits, and knowledge provenance. It distinguishes the Council’s reference agent from independent third-party agents.

### IX.1 Purpose and Scope

Software agents — including large language models, scripted bots, and hybrid systems — can assist users with **education, navigation, liquidity management, merchant onboarding, support, and operational workflows** connected to Stables. Integration may occur through **application interfaces, APIs, messaging platforms, and future agent-native protocols**.

The Stables protocol is **deterministic**; AI is **not** a source of monetary truth. Agents are **interfaces and interpreters**, not issuers of liabilities and not substitutes for participant judgment where value or risk is at stake.

### IX.2 Open Integration and Invariant Boundaries

**Third-party agents are welcome** to build on top of Stables, subject to the same open-platform posture as other applications (see Article I.4). Integrators may offer automation, analytics, and user experience layers, provided they:

- **respect all immutable monetary rules** — no agent may bypass, override, or obscure solvency, issuance, redemption, or oracle logic encoded in the core;
- **not present discretionary protocol control** as if it were available to the agent or user;
- **separate execution from advice** — high-impact actions (e.g. large transfers, collateral moves, contractual commitments) should remain clearly attributable to the **human or programmatic principal** with appropriate confirmation, as implementation allows.

Agents may **read** public and permissioned state to inform users; they may **initiate** transactions only through **normal participant-controlled interfaces** and credentials, not through hidden privilege.

### IX.3 Disclosure, Provenance, and Conduct

Agents that interact with users **on behalf of** or **in connection with** Stables-related services shall:

- **identify themselves as non-human** where material to the interaction, so users are not deceived about the nature of the counterparty;
- **disclose** when responses are generated by models that may err, and point to **authoritative sources** (protocol documentation, on-chain state, Council communications) for verification;
- **avoid** presenting speculation, incentives, or unverified claims as **official Council or protocol guarantees**.

The Council may publish **reference materials and constraints** for agent builders (e.g. allowed APIs, disclosure templates). Those materials are operational, not part of the immutable core, unless separately anchored.

### IX.4 Council Agent and Third-Party Agents

The Council may operate or endorse a **reference community agent** (including **StablesAgent**) for knowledge distribution, support, and neutral explanation of governance outcomes, as described in Article VII. That agent is **one implementation** among many: **independent agents and integrators remain essential** to a competitive, resilient ecosystem.

No exclusivity is claimed over AI integration. Third-party agents compete on **quality, transparency, and user alignment**, not on access to the monetary core.
