...
## 4. The Transition Doctrine
Stables does not claim to be the final form of human money. It is a necessary bridge from the fiat present to a sovereign future.
- **Stage 1 (Present - Merchant Opt-In):** Because the world still runs on fiat pricing, Stables provides synthetic pegged assets (USDs, EURs) so that merchants and users can opt-in to a sovereign, zero-fee network today without abandoning familiar pricing.
- **Stage 2 (Minima-Native):** As the network grows and Minima distribution widens, the reliance on fiat pegs will fade as participants begin pricing goods directly in Minima.
- **Stage 3 (The Circular Horizon):** A future state where monetary power is recognized as a fundamental human right, flowing continuously rather than accumulating in centralized silos. Stables is building the infrastructure to make this horizon accessible.
