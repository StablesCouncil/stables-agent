...
- **Connect node + MDS 500:** Optional **Hub session uid** in the Connect modal (same as demo); **`mds.js`** reads **`window.__STABLES_MDS_SESSION_UID`** when **`MDS.DEBUG_HOST`** is set. See **`prod_stables_app_demo/CHANGELOG.md`**.

- **`mds.js` Connect-node scheme:** With **`MDS.DEBUG_HOST`** and port **9003**, use **HTTPS** for **`mainhost`** / **`filehost`** even when the page is **`file://`** or **`http://`**, matching Minima MDS on **:9003**. (Mirrors demo fix; see **`prod_stables_app_demo/CHANGELOG.md`**.)
