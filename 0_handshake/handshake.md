...
- **Button hierarchy (site + MiniDapp shell)**: At most one **`btn-primary`** per obvious visual group; use **`btn-secondary`** for supporting actions. Do not use **`btn-primary`** for disabled or “coming soon only” states. Normative detail: **`0_handshake/web_component_spec.md`** (COMPONENTS → Buttons, including **Button hierarchy (mandatory)**).
