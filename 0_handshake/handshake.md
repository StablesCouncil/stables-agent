...
**Last Calibration**: 2026-04-14 (v12)
...
- **Packaging Rule**: Zip the *contents* of the **chosen** `prod_stables_app_...` folder (showcase or demo) directly to create the `.mds.zip`. **Exclude** that folder’s `build/` directory (generated zips + notes). No file renaming inside the zip. **Showcase** packages mirror the GitHub **`dapp/`** layout: **`dapp.conf`** at zip root with **`web`** = **`showcase/index.html`**, sibling **`assets/`**, **`showcase/index.html`**, root art (**`agent.png`**, **`stables_icon.png`**, …), and optional root **`index.html`** redirect.
