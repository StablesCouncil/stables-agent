...
**Markup** (implemented in `prod_stables_app_v00.00.00.00.03/index.html`; see `minidapp_version.md` if the folder bumps):
...
**Reference**: `1_development/stream_1_app/prod_stables_app_v00.00.00.00.03/index.html` (`.app-section--caption-bottom`, `.stitle-row`, `.agent-mini-btn`, `.app-section-card` in the page `<style>` block).
...
| Piece | Contract |
|-------|----------|
| **Class** | `welcome-modal-showcase-intro` on the text container (`#welcomeShowcaseIntroBody` in active `prod_stables_app_*/index.html`). |
...
**Rule**: Pages that share the **site chrome** (header, side rail, StablesAgent FAB, consistent OG/Twitter meta) SHOULD be authored as Eleventy templates under **`src/`**, with chrome in **`src/_includes/`**, so one partial change updates every compiled page. Hand-maintained pages live under **`webpages/`** and ship via **`site/`** until migrated; prefer Eleventy for new long-form or chrome pages when you adopt that pipeline.

| Piece | Contract |
|-------|------------|
| **Build** | In that folder: `npm install` (once per machine), then **`npm run build`** when you use Eleventy templates. The **hand-maintained** public tree is built with **`npm run sync:site`**, which writes **only** into **`site/`** from **`webpages/`** and **`static/`**. Ship **`site/`** contents to the Pages repo root. Run the right command before copying or pushing when you changed **`src/`** or **`webpages/`** / **`static/`**. |
| **Config** | **`.eleventy.js`** — `input: "src"`, `includes: "_includes"`, output at this folder’s root by default. Shared shipped files (`stables.css`, `assets/`, `brand/`, `CNAME`, images) live under **`static/`** and are copied into **`site/`** by **`sync:site`**; do not duplicate them at the task folder root. |
...
| **Hand-maintained exception** | **Playing Field** is authored under **`webpages/pages/playing_field/index.html`** and emitted to **`site/playing_field.html`** by **`npm run sync:site`**. It is **not** overwritten by Eleventy unless you later promote it to **`src/`**. Reuse **`src/_includes/`** patterns by copy or by adding a new template when you promote another page to the Eleventy pipeline. |
| **Site chrome body classes** | Long-form static pages that use the shared header and rail: **`site-chrome-body`**, **`deck-chrome-page`** (taller header, same scale as **`playing_field.html`**), **`links-page-body`** (brand and MINIMA pill aligned to the same horizontal column as the main content, see **`static/assets/site-chrome.css`** in the sandbox). Default column width for that math is **680px** (All links hub). Pages whose main column is **`site-chrome-main--document`** (max **960px**) add **`playing-field-page`**, **`qr-code-page`**, or **`circulareconomy-page`** on **`<body>`** so **`--links-stack-max`** becomes **960px**. **`ambassadors-doc-page`** uses **860px** to match that page’s **`.container`**. **`has-site-rail`** when the globe/share rail is present. At **deploy root** (the **`site/`** tree and the live Pages root), top-level HTML links **`stables.css`** and **`assets/…`**; one level down (e.g. **`circulareconomy/index.html`**) use **`../stables.css`** and **`../assets/…`**. |
