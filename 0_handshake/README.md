...
| **`1_development/stream_1_app/README.md`** | Every model shipping **Pages** or **MiniDapp web** | **Dev drive map:** `task_stablescouncil_github_io/`, **`dapp/`** (live **`/dapp/`** mirror), **`prod_stables_app_*`** product trees; sync + demo **`robocopy`** refresh. Cross-links **`handover_document.md`** (Publication parity checklist). |
